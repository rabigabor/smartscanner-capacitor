...
public class MrtdTd1: MrzRecord{
    ...
    override public func fromMrz(_ mrz: String) throws {
        ...
        validDocumentNumber = try p.checkDigit(14, 0, try MrzRange(5, 14, 0), "document number");
        documentNumber = try p.parseString(try MrzRange(5, 14, 0));

        optional = try p.parseString(try MrzRange(15, 30, 0));
        dateOfBirth = try p.parseDate(try MrzRange(0, 6, 1));
        validDateOfBirth = try p.checkDigit(6, 1, try MrzRange(0, 6, 1), "date of birth") && dateOfBirth.isDateValid();
        sex = try p.parseSex(7, 1);
        expirationDate = try p.parseDate(try MrzRange(8, 14, 1));
        validExpirationDate = try p.checkDigit(14, 1, try MrzRange(8, 14, 1), "expiration date") && expirationDate.isDateValid();
        nationality = try p.parseString(try MrzRange(15, 18, 1));
        optional2 = try p.parseString(try MrzRange(18, 29, 1));
        validComposite = try p.checkDigit(29, 1, p.rawValue([try MrzRange(5, 30, 0), try MrzRange(0, 7, 1), try MrzRange(8, 15, 1), try MrzRange(18, 29, 1)]), "mrz");
    ...
    override public func toMrz() throws -> String {
        ...
        let dno = MrzParser.toMrz(documentNumber, 9) + (try MrzParser.computeCheckDigitChar(MrzParser.toMrz(documentNumber, 9))) + MrzParser.toMrz(optional, 15);
        sb.append(dno);
        sb.append("\n");
        // second line
        ...
        sb.append(try MrzParser.computeCheckDigitChar(dno + dob + ed + MrzParser.toMrz(optional2, 11)));
        sb.append("\n");
        // third line
